<template>
    <div :class="{ chat_main: true, position_css: position }" v-show="hide">
        ...
        <div
            v-show="small"
            :class="{
                small_socket_status: socketStatus,
                small_main: true,
                shadow: true,
            }"
        >
        ...
        <div v-show="!small" :class="{ main_block: true }">
            ...
            <div class="left_block">
                <ul>
                    <li
                        v-for="(v, k) in data.friends"
                        :key="k"
                        @click="openChatItem(v.id)"
                        :class="{ ck: v.id == data.selectId }"
                    >
                        <div class="content_item">
                            <img
                                class="avatar"
                                :src="
                                    data.selfData.sid == v.sid &&
                                    data.selfData.stype == v.stype
                                        ? (v.rinfo && v.rinfo.avatar) ||
                                          require('@/assets/Home/kefu.png')
                                              .default
                                        : (v.sinfo && v.sinfo.avatar) ||
                                          require('@/assets/Home/kefu.png')
                                              .default
                                "
                            />
                            <div class="avatar_right">
                                <div class="nickname text_over">
                                    {{
                                        data.selfData.sid == v.sid &&
                                        data.selfData.stype == v.stype
                                            ? (v.rinfo && v.rinfo.nickname) ||
                                              "anonymous"
                                            : (v.sinfo && v.sinfo.nickname) ||
                                              "anonymous"
                                    }}
                                </div>
                                <div class="text text_over">
                                    {{
                                        v.lastMsg
                                            ? v.lastMsg.content_type &&
                                              v.lastMsg.content_type == "text"
                                                ? v.lastMsg.content
                                                : "[ " +
                                                  v.lastMsg.content_type +
                                                  " ]"
            ...
            <div class="right_block">
                <div class="top_title">
                    <span v-show="closeVis" @click="closeChat"
                        ><el-icon><Close /></el-icon></span
                    >{{
                        loadInfoById(data.selectId)["nickname"] || "Loading..."
                    }}
                </div>
                <div class="chat_content">
                    <el-scrollbar
                        ...
                        v-if="data.selectId > 0"
                    >
                        <div class="scrollbar_inner" ref="scrollbarDivRef">
                            <div
                                class="item"
                                v-for="(v, k) in data.chatList"
                                :key="k"
                            >
                                <img
                                    class="avatar"
                                    :src="
                                        loadInfoBySid(v.sid, v.stype).avatar ||
                                        require('@/assets/Home/kefu.png')
                                            .default
                                    "
                                />
                                <div class="avatar_right">
                                    ...
                                    <div class="text">
                                        <div v-if="v.content_type == 'text'">
                                            {{ v.content || "-" }}
                                        </div>
                                        <div v-if="v.content_type == 'image'">
                                            <el-image
                                                :src="v.content"
                                                fit="cover"
    ...
    setup(props) {
        ...
        const send = (sendType) => {
            ...
            let params = {
                ...
                content:
                    sendType == "text" ? data.params.content : data.params.file,
                token: props.params.token || "",
            };
            loading.value = true;
            proxy.R.post("/Chat/send", params)
                .then((res) => {
                    if (!res.code) {
                        if (sendType == "text") {
                            data.params.content = "";
        ...
        const loadInfoById = (id) => {
            ...
            if (
                data.selfData.sid == selectData.sid &&
                data.selfData.stype == selectData.stype
            ) {
                ...
                setTimeout(() => {
                    if (proxy.$refs.scrollbarRef)
                        proxy.$refs.scrollbarRef.setScrollTop(
                            parseFloat(proxy.$refs.scrollbarDivRef.clientHeight)
                        );
                }, 300);
            });
        };
