    ...
    <div class="qw_table">
        <transition name="el-zoom-in-top"
            ><layout-search
                ...
                :dictData="dialogParams.dictData"
                @data="searchData"
        /></transition>
        <div class="table_top">
                    ...
                    <template #default="scope">
                        ...
                        <span v-if="v.type == 'tags_array'"
                            ><q-tags
                                ...
                                :value="arrItem"
                        /></span>
                        <span v-if="v.type == 'icon_tags'"
                            ><i
                                :class="
                                    'icon_tags fa ' + scope.row[v.value]
                        ...
                        <el-image
                            ...
                            :style="
                                v.style || {
                                    ...
                                    textAlign: 'center',
                                    lineHeight: '65px',
                                    background: R.isEmpty(scope.row[v.value])
                                        ? '#f2f2f2'
                                        : 'null',
                                    display: 'block',
                                }
                            ...
                            lazy
                            :preview-src-list="
                                v.perView ? [scope.row[v.value]] : null
                            "
                            :preview-teleported="v.perView ? true : false"
                        >
                            <template #error>
                                <el-icon
                        ...
                        <slot name="table_column_button">
                            <div class="table_col_handle">
                                <el-button
                                    :title="$t('btn.view')"
                                    v-if="
                                        btnConfigs.show && btnConfigs.show.show
                                    "
                                    :icon="View"
        ...
        >
            <template #header>
                ...
                <span
                    class="fullscreenbtn"
                    @click="dialogParams.fullscreen = !dialogParams.fullscreen"
            ...
            <slot name="table_add_hook" :dialogParams="dialogParams">
                <el-form
                    v-if="
                        dialogParams.add && dialogParams.add.column.length > 0
                    "
                    ref="addForm"
        ...
        <!-- 编辑dialog -->
        <el-dialog
            :destroy-on-close="dialogParams.destroyOnClose || true"
            ref="editDialog"
            custom-class="table_dialog_class"
            v-model="editVis"
            :title="$t('btn.edit')"
        ...
        >
            ...
            >
                <el-form
                    v-if="
                        dialogParams.edit && dialogParams.edit.column.length > 0
                    "
                    ref="editForm"
                    label-position="right"
                    :rules="dialogParams.rules || null"
        ...
        >
            ...
            >
                <el-form
                    v-if="
                        dialogParams.view && dialogParams.view.column.length > 0
                    "
                    label-position="right"
                    :label-width="dialogParams.labelWidth"
                >
                            ...
                            ><div class="table-form-content">
                                ...
                                >
                                    ...
                                    <div
                                        ...
                                        v-html="
                                            editorSplit(formData.view[v.value])
                                        "
                                    ></div>
                                    <span v-if="v.viewType == 'tags_array'"
                                        ><q-tags
                                            :styles="{ marginRight: '8px' }"
                                            v-for="(arrItem, arrKey) in formData
                                                .view[v.value]"
                                            :key="arrKey"
                                            :value="arrItem"
                                    /></span>
                                    ...
                                    <div v-if="v.viewType == 'images'">
                                        <el-image
                                            v-for="(imgItem, imgKey) in formData
                                                .view[v.value]"
                                            class="images_item"
                                            :key="imgKey"
                                            :style="
    ...
    setup(props, { emit }) {
        ...
        let dialogParam = {
            ...
            loadView: async (url = null) => {
                // 获取详情信息
                return await proxy.R.get(
                    url != null ? url : pageUrl + "/" + row[props.columnId]
                );
            },
            add: {
            ...
            dialogParam.dict.map(async (item) => {
                ...
                if (item.selectDictByColumId && listData.listData.length > 0) {
                    listData.listData.map((listItem) => {
                        if (
                            !proxy.R.isEmpty(listItem[item.name]) &&
                            listItem[item.name] != 0
                        )
                            columnIds.push(listItem[item.name]);
                    });
                }
                if (item.selectDictByColumId) {
                    if (!proxy.R.isEmpty(columnIds) && columnIds.length > 0)
                        dictUrlParams[props.columnId] =
                            _.uniq(columnIds).join(",") + "|in";
                }
                let dictResp = await proxy.R.get(item.url, dictUrlParams);
                dialogParams.dictData[item.name] = item.isPageDict
                    ? dictResp["data"]
                    : dictResp;
                if (item.addSelect)
                    dialogParams.dictData[item.name].unshift(item.addSelect);
            });
        };
        ...
        const dictFind = (
            ...
            let dictVal = "-";
            if (
                dialogParams.dictData[dictName] &&
                dialogParams.dictData[dictName].length > 0
            ) {
                dialogParams.dictData[dictName].map((item) => {
                    if (item[valueName] == value) dictVal = item[labelName];
                    if (item.children && item.children.length > 0) {
                        item.children.map((itemS) => {
                            if (itemS[valueName] == value)
                                dictVal = itemS[labelName];
                            if (itemS.children && itemS.children.length > 0) {
                                itemS.children.map((itemT) => {
                                    if (itemT[valueName] == value)
                                        dictVal = itemT[labelName];
                                });
                            }
                        });
                    }
        ...
        watch(
            ...
            (e) => {
                if (props.tableData.data) {
                    ...
                    listParams.per_page = parseInt(
                        props.tableData.per_page || 1
                    );
                    listParams.last_page = parseInt(
                        props.tableData.last_page || 1
                    );
                    listParams.page = parseInt(
                        props.tableData.current_page || 1
                    );
                    return;
                }
                loadData();
        ...
        const lazyLoad = async (tree, treeNode, resolve) => {
            let childRaw = await getChildrenData(
                tree[props.tableCfg.pid || "id"]
            );
            // 先循环判断是否存在过
            if (lazyDataTmp.data.length == 0)
                lazyDataTmp.data.push({
                    tree,
                    treeNode,
                    resolve,
                    id: tree[props.columnId],
                });
            lazyDataTmp.data.map((lazyItem, lazyKey) => {
                if (
                    lazyItem.tree &&
                    lazyItem.tree[props.columnId] == tree[props.columnId]
                ) {
                    lazyDataTmp.data[lazyKey] = {
                        tree,
                        treeNode,
                        resolve,
                        id: tree[props.columnId],
            ...
            if (childRaw.length > 0) {
                childRaw.map((cItem) => {
                    let rowStr =
                        "" +
                        cItem[props.columnId] +
                        "|" +
                        cItem[props.tableCfg.lazyPid || "pid"];
                    if (lazyDataTmp.loadRow.indexOf(rowStr) == -1)
                        lazyDataTmp.loadRow.push(rowStr);
                });
            }
        ...
        const lazyAutoUpdate = (handleType) => {
            try {
                ...
                lazyDataTmp.data.map((item) => {
                    if (
                        formData[handleType][props.tableCfg.lazyPid || "pid"] ==
                        item.tree[props.columnId]
                    ) {
                ...
                pidArr.map((pidArrItem) => {
                    ...
                    if (parentIdVal != -1) {
                        lazyDataTmp.data.map(async (item) => {
                            if (parentIdVal == item.tree[props.columnId]) {
                                proxy.$refs.multipleTable.store.states.lazyTreeNodeMap.value[
                                    parentIdVal
                                ] = [];
                                await lazyLoad(
                                    item.tree,
        ...
        const showData = async (row) => {
            ...
            const newViewData = await proxy.R.get(
                pageUrl + "/" + row[props.columnId]
            );
        ...
        const openEditDialog = async (row) => {
            ...
            formData.edit = {};
            editVis.value = true;
            formData.edit = await proxy.R.get(
                pageUrl + "/" + row[props.columnId]
            );
            await dialogParams.edit.column.map((item, key) => {
                // placeholder关联字段
                if (item.relation)
                    dialogParams.edit.column[key]["placeholder"] =
                        formData.edit[item.relation];
            });
        ...
        const updateData = () => {
            proxy.$refs.editForm.validate((valid) => {
                ...
                try {
                    proxy.R.put(
                        pageUrl + "/" + formData.edit[props.columnId],
                        formData.edit
                    )
                        .then((res) => {
                            loading.value = false;
        ...
        const searchData = (e) => {
            ...
            Object.keys(listParams).map((delVal) => {
                if (proxy.R.isEmpty(listParams[delVal]))
                    delete listParams[delVal];
            });
        ...
        const excelExport = () => {
            ...
            if (dialogParam.exportData.length > 0)
                exportOptions = dialogParam.exportData;
            if (dialogParam.exportDataAdd.length > 0)
                exportOptions = _.concat(
                    exportOptions,
                    dialogParam.exportDataAdd
                );
            // 显示字段
            ...
            listData.listData.map((item) => {
                ...
                if (
                    ...
                    exportOptions.map((itemOption) => {
                        ...
                        if (
                            itemOption.type == "dict_tags" ||
                            itemOption.type == "dict"
                        ) {
                            colItemVal = dictFind(
                                itemOption.value,
                                colItemVal,
                                itemOption.labelName || "label",
                                itemOption.valueName || "value"
                            );
                        }
            ...
            XLSX.writeFile(
                wb,
                (store.state.load.routeMenuName || "table") + ".xlsx"
            );
        };
