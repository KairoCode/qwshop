<template>
    <div class="qw_input">
        <q-font
            ...
            v-model:value="data.modelFormValue"
        />
        <table-select
            v-if="params.type == 'table_select'"
            v-model:value="data.modelFormValue"
            :params="params"
        ...
        <el-input
            ...
            :show-word-limit="params.showWordLimit || true"
        ...
        >
            <img
                v-if="data.modelFormValue"
                style="width: 100%; height: 100%"
                :src="data.modelFormValue"
        ...
        >
            <span
                v-if="data.modelFormValue"
                :title="data.modelFormValue"
                class="file_path"
                >{{ data.modelFormValue }}</span
            >
        ...
        <div
            class="editor"
            v-if="params.type == 'editor'"
            :style="'height:' + (params.height || '385px')"
        >
            <div :id="'toolbar' + rand"></div>
            <div
                class="editClass"
                :style="'height:' + (params.height || '300px')"
                :id="'editor' + rand"
            ></div>
        </div>
        <el-switch
        ...
        />
        <el-date-picker
            v-if="params.type == 'daterange'"
            style="width: 100%; box-sizing: border-box"
            type="daterange"
            value-format="YYYY-MM-DD HH:mm:ss"
            :start-placeholder="
                params.startPlaceholder || $t('btn.dateRangeStart')
            "
            :end-placeholder="params.endPlaceholder || $t('btn.dateRangeEnd')"
            v-model="data.modelFormValue"
        />
        <el-date-picker
...
import qFont from "./font.vue";
import tableSelect from "./table_select.vue";
...
export default {
    ...
    setup(props, { emit }) {
        ...
        watch(
            ...
            (e) => {
                ...
                if (props.params.type == "editor") {
                    if (
                        objects.editorObj !== null &&
                        objects.E !== null &&
                        editorHandle(e) != ""
                    ) {
                        if (objects.editorObj.getHtml() != editorHandle(e)) {
                            objects.E.SlateTransforms.removeNodes(
                                objects.editorObj,
                                { at: [0] }
                            );
        ...
        const lazyLoad = async (node, resolve) => {
            ...
            if (node.level == 0) {
                return resolve(
                    await proxy.R.get(props.params.lazyUrl + "&pid=0")
                );
            } else {
                const data = await proxy.R.get(
                    props.params.lazyUrl +
                        "&pid=" +
                        node.data[props.params.idName || "code"]
                );
                resolve(data);
            }
        };
        ...
        if (
            props.params.type == "cascader_lazy" &&
            props.params.props &&
            props.params.props.lazy
        ) {
        ...
        const getEditDom = async () => {
            ...
            nextTick(() => {
                ...
                editorConfig.placeholder =
                    props.params.placeholder || proxy.$t("btn.inputContent");
