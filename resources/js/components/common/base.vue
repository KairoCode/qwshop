<template>
    <div class="qwbase">
        <el-container>
            ...
            <el-aside class="aside_class" :width="collapse ? '63px' : '200px'">
                <div class="sys_logo">
                    <span v-if="collapse"
                        ><img
                            :src="
                                require('@/assets/Admin/system_logo.png')
                                    .default
                            "
                            :alt="$t('config.systemName')"
                    /></span>
                    <span v-if="!collapse"
                        ><div class="sys_span">
                            <img
                                ...
                                "
                                :alt="$t('config.systemName')"
                            /><em>{{ $t("config.systemName") }}</em>
                        </div></span
                    >
                </div>
                <el-menu
                    :class="{ sub_menu: !collapse, sub_menu2: collapse }"
                    :collapse="collapse"
                    :default-active="nowRoutePointIds[0] || 0"
                    v-if="
                        menuData[routeUriIndex] &&
                        menuData[routeUriIndex][moduleIndex] &&
                        menuData[routeUriIndex][moduleIndex].children
                    "
                >
                    <el-scrollbar>
                        <div
                            v-for="v in (menuData[routeUriIndex] &&
                                menuData[routeUriIndex][moduleIndex] &&
                                menuData[routeUriIndex][moduleIndex]
                                    .children) ||
                            []"
                            ...
                            <el-sub-menu :index="v.id + ''" v-if="v.children">
                                <template #title>
                                    <el-icon
                                        ...
                                        ></i
                                    ></el-icon>
                                    <span class="menu_i" v-if="!collapse">{{
                                        v.name
                                    }}</span>
                                </template>
                                <el-menu-item
            ...
            <el-container>
                ...
                <el-header class="modlue_header">
                    <div class="fold" @click="collapseChange">
                    ...
                    <div class="module_content">
                        <el-scrollbar>
                            <ul class="module_ul">
                                <li
                                    v-for="(v, k) in menuData[routeUriIndex] ||
                                    []"
                                    :key="k"
                                    @click="
                                        openWin(v);
                                        selectModuleHandle(k);
                                    "
                                    :class="
                                        v.checked ||
                                        (nowRoutePoint.length > 0 &&
                                            nowRoutePoint[
                                                nowRoutePoint.length - 1
                                            ].key == k)
                                            ? 'ck'
                                            : ''
                                    "
                        ...
                        <div class="nickname">
                            <el-dropdown popper-class="custom_dropdown">
                                <span class="el-dropdown-link">
                                    ...
                                    <el-icon class="icon_class" :size="12"
                                        ><arrow-down
                                    /></el-icon>
                                </span>
                                <template #dropdown>
                ...
                <el-main class="el_main_css">
                    <el-scrollbar>
                        <div v-if="!hideMian" class="sys_main_content shadow">
                            <!-- 具体内容 -->
                            <div v-if="!hideTitle" class="mian_title">
                                <slot name="main_title">{{
                                    routeMenuName || "Menu Name"
                                }}</slot>
                            </div>
                            <slot v-if="!hideTitle" name="main_line"
        ...
        <el-dialog
            destroy-on-close
            custom-class="base_dialog_class"
            v-model="editUserVis"
                ...
                <el-row :gutter="20">
                    <el-col :span="24">
                        <el-form-item>
                            ...
                            >
                            <el-button @click="editUserVis = false">{{
                                $t("btn.cancel")
                            }}</el-button>
                        </el-form-item>
                    </el-col>
...
const openWin = (row) => {
    if (row.apis != "") {
        // 看是否是跳转外链的存在
        if (
            row.apis.indexOf("http://") != -1 ||
            row.apis.indexOf("https://") != -1
        ) {
            _open(row.apis, true);
        } else {
            ...
            if (row.view != "" && router.hasRoute(row.apis)) {
                ...
                if (
                    ...
                    console.error(
                        "view:" + row.view,
                        "router has " + router.hasRoute(row.apis),
                        "children has " +
                            (row.children && row.children.length) || "empty"
                    ); // 异常信息提醒
                }
            }
        }
...
const updateUser = async () => {
    proxy.$refs.userEdit.validate(async (valid) => {
        ...
        try {
            const servUser = await store.dispatch(
                "login/editUserSer",
                userEditForm
            );
            if (!servUser.code) {
